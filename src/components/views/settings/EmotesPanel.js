...
export default class EmotesPanel extends React.Component {
    ...
    _getEventContent() {
        if (this.props.roomId) {
            ...
            if (thisRoom) {
                const event = thisRoom.currentState.getStateEvents('minority.destroyer.room_emotes', this.props.stateKey || "");
                if (event && !event.error && event.event.content) {
                    return event.event.content;
                }
            }
        } else {
            const event = MatrixClientPeg.get().getAccountData('minority.destroyer.user_emotes');
            if (event && !event.error && event.event.type == 'minority.destroyer.user_emotes' && event.event.content) {
                return event.event.content;
            }
        }
    }
    ...
    _saveEmotes() {
        ...
        let promise;
        if (this.props.roomId) {
            promise = MatrixClientPeg.get().sendStateEvent(this.props.roomId, 'minority.destroyer.room_emotes', emotes, this.props.stateKey || '');
        } else {
            promise = MatrixClientPeg.get().setAccountData('minority.destroyer.user_emotes', emotes);
        }
        promise.catch((e) => {
